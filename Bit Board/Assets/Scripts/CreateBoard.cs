...
public class CreateBoard : MonoBehaviour
{
    ...
    void printboard(string name, long BB)
    {
    ...
    void Update()
    {
        if(Input.GetMouseButtonDown(0))
        {
            ...
            {
                int rp = (int)hit.collider.gameObject.transform.position.z;
                int cp = (int)hit.collider.gameObject.transform.position.x;

                if(GetCellState((dirtBB | desertBB) & (~treeBB), rp, cp))
                {
                    GameObject house = Instantiate(housePrefab);
                    house.transform.parent = hit.collider.gameObject.transform;
                    house.transform.localPosition = Vector3.zero;
                    playerBB = SetCellState(playerBB, rp, cp);
                    CalculateScore();
                }
            }
        }
